...
##### USER INPUT #####
    ...
    getwd()
    setwd("~~~~~SET HERE~~~~~") # set to your desired working folder (directory)
    ...
    data <- read.csv("~~~YOUR CSV FILE~~~.csv", row.names = NULL) # if the first column contains names for each row, then change "row.names = NULL" to "row.names = 1" 
    data

    # Give a name for your output .fcs file (don't foret to add '.fcs' at the end of the name)
    fcsfilename <- "~~~FILE NAME AFTER TURNING~~~.fcs"
...
##### END USER INPUT #####
    ...
    row.names(metadata)=paste('$P',1:nrow(metadata),sep="")#the rowname of FCS metadata must be $Pn
